...
\begin{proof}
  For each integer row operation on an $m\times n$ matrix, there is a corresponding matrix $P\in\GL_m(\Z)$ such that the effect of applying the row operation is equivalent to left multiplying by $P$. Moreover these ``elementary'' matrices generate $\GL_m(\Z)$. Similarly integer column operations correspond to right multiplying by matrices in $\GL_n(\Z)$.
\end{proof}
...
We will establish a similar normal form which distinguishes the isomorphism types of $\Z^n/S(A)$ for $m\times n$ matrices. This is known as the Smith Normal Form (SNF).
...
Here is an example integer $3\times 5$ matrix in Smith Normal Form
\[ A \coloneqq
  \begin{bmatrix}
    2 & 0 & 0 & 0 & 0 \\
    0 & 4 & 0 & 0 & 0 \\
...
\begin{theorem}
  Let $A$ be an $m\times n$ integer matrix in SNF, with invariant factors $d_1\mid d_2\mid\cdots\mid d_r$. Then $\Z^n/S(A)\cong\Z_{d_1}\oplus\cdots\oplus\Z_{d_r}\oplus\Z^{n-r}$.
\end{theorem}

\begin{theorem}
  \label{SNF-existence}
  If $A$ is an $m\times n$ integer matrix, then there is an $m\times n$ integer matrix $B$ with $A\approx B$ and $B$ in Smith Normal Form.
\end{theorem}
...
\begin{proof}
  ...
  \]
  with $d>0$. Let $A_{i,j}$ be a nonzero entry of $A$. Swap rows $i$ and 1, and columns $j$ and 1 if required, to ensure $i=j=1$, and multiply row 1 by $-1$ if necessary to ensure $A_{1,1}>0$. If $A_{1,1}$ divides all entries in row and column 1, then we can  multiples of row/column 1 to the other rows/columns, to reach the form ($\star$).

  If $A_{1,1}$ does not divide all entries in row and column 1, then we can decrease this entry as follows. Suppose that $A_{1,1}\nmid A_{i,1}$ for some $i$. Write $A_{i,1}=qA_{1,1}+r$ with $0<r<A_{1,1}$. Then add $-q$ times row 1 to row $i$, and swap rows 1 and $i$. Similarly, if $A_{1,1}\nmid A_{1,j}$ for some $j$, then write $A_{1,j}=qA_{1,1}+r$ with $0<r<A_{1,1}$. Then add $-q$ times column 1 to column $j$, and swap columns 1 and $j$.

  Therefore, after repeating this enough times, we can ensure $A_{1,1}$ divides all entries in row and column 1, and thus we can reach the block form ($\star$). Inductively, we can reduce $A'$ to SNF by row and column operations, so we can reduce $A$ to a matrix with block form
  \[ \left[
      \begin{array}{ccc|cc}
        d_1 &&&& \\
            & \ddots &&& 0 \\
  ...
  \[
    ...
    \end{bmatrix}.
  \]
  In this way, we can use row and column operations to replace $d_i$ and $d_j$ with $\gcd(d_i,d_j)$ and $\lcm(d_i,d_j)$ respectively. Repeating this leaves us with a matrix in Smith Normal Form.
\end{proof}

We in fact have uniqueness from the following important theorem
...
Note that we have proven the existence of such integers, by the previous result. For the uniqueness, we refer the reader to \cite{dummit}.
...
The proof of \autoref{SNF-existence} is readily transformed into a procedure for computing the Smith Normal Form of a given matrix. Here is such a procedure written in {\sc Magma}.\newline
...
\begin{definition}
  For an $m\times n$ integer matrix $A$, and $0\leq k\leq m,n$, let $D_k(A)$ denote the gcd of all determinants of $k\times k$ submatrices of $A$. We adopt the convention that the determinant of a $0\times0$ matrix is 1, so that $D_0(A)=1$ for any integer matrix $A$.
\end{definition}
...
We can use this to get an easily computable bound of the determinant of any square submatrix of an $m\times n$ matrix $A$.
...
Using the ideas presented in the previous section, we are able to produce an improved Smith Normal Form algorithm. Here is an implementation in {\sc Magma}.\newline
